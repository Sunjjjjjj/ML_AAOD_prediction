...
for lo in ['land', 'ocean']:
    ...
    plt.savefig(figdir + 'Climatology_SSA_%s.png' % (lo), dpi = 300, transparent = True)  
...
#%%
    ...
    plt.savefig(figdir + 'Validation_%s.png' % (titles[i]), dpi = 300, transparent = True)        
...
#%%
    ...
    cb = plt.colorbar(cax = cax, fraction=0.1, pad=0.05, shrink = 0.9, aspect = 10, \
                  label = '# num', extend = 'both', )
    plt.savefig(figdir + 'Validation_screened.png', dpi = 300, transparent = True)        
