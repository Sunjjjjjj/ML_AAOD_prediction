...
for ilayer in hidden_layers:
    for ineuron in layer_size:
        ...
        for ifold in np.arange(Kfold):
            ...
            expName = "%s_%i-layer_%i-neuron" % ('DNN_AAOD_F11', ilayer, ineuron)
            train_log_path = scriptDir + "train_log/%s_train_log/fold-%02i" % (expName, ifold)
            try:
...
# =============================================================================
        ...
        #                     verbose=1, callbacks=[check_point, early_stop])
            ...
            # save the best model
            tf.saved_model.save(model, scriptDir + "best_model/best_model_%s_fold-%i" % (expName, ifold))
# =============================================================================
#  validation
# =============================================================================
            # load best model 
            model = tf.keras.models.load_model(scriptDir + "best_model/best_model_%s_fold-%i" % (expName, ifold))
            # prediction
        ...
        model_performance[expName] = train_res

np.save(scriptDir + 'model_performance_F11', model_performance)
...
#%%
model_performance = np.load(scriptDir + 'model_performance_F11.npy', allow_pickle = True).item()
import re
...
for i, iexp in enumerate(model_performance.keys()):
    ...
    num_f, num_l, num_n = re.findall('\d+', iexp)
...
temp_m.to_csv('model_performance_F11.csv')
