...
expName = "%s_%i-layer_%i-neuron" % ('DNN_AAOD_train_ST3', 4, 2**6)
...
for lo in ['land', 'ocean']:
    ...
    plt.savefig(figdir + 'Climatology_AAOD_%s_ST.png' % (lo), dpi = 300, transparent = True)  
# #%%
# cmap = matplotlib.cm.cubehelix_r
...
for lo in ['land', 'ocean']:
    ...
    plt.savefig(figdir + 'Climatology_SSA_%s_ST.png' % (lo), dpi = 300, transparent = True)  
...
plt.savefig(figdir + 'Time_series_AAOD.png', dpi = 300, transparent = True)     
...
plt.savefig(figdir + 'Time_series_SSA.png', dpi = 300, transparent = True)   
...
for j, iROI in enumerate(ROIs.keys()): 
    ...
    if j == 0:
        plt.legend(ncol = 3)
    # print(iROI, X1.corr(X2), X1.corr(X3))
plt.savefig(figdir + 'Time_series_AOD.png', dpi = 300, transparent = True)   
...
#%%
    ...
    plt.savefig(figdir + 'Validation_%s_ST.png' % (titles[i]), dpi = 300, transparent = True)        
...
#%%
    ...
    plt.savefig(figdir + 'Validation_SSA_sensitivity_to_AOD.png', dpi = 300, transparent = True) 
    ...
    cb = plt.colorbar(cax = cax, fraction=0.1, pad=0.05, shrink = 0.9, aspect = 10, \
                  label = '# num', extend = 'both', ticks = [1, 10, 1e2, 1e3] )
    plt.savefig(figdir + 'Validation_SSA_sensitivity_to_UVAI.png', dpi = 300, transparent = True)   
#%%
    # validation screened
    paras = {'MODIS': 'Y_pred',
         'OMAERUV': 'AAOD500',
    ...
    cb = plt.colorbar(cax = cax, fraction=0.1, pad=0.05, shrink = 0.9, aspect = 10, \
                  label = '# num', extend = 'both', )
    plt.savefig(figdir + 'Validation_screened_ST.png', dpi = 300, transparent = True)        
    ...
    cb = plt.colorbar(cax = cax, fraction=0.1, pad=0.05, shrink = 0.9, aspect = 10, \
                  label = '# num', extend = 'both', )
    plt.savefig(figdir + 'Validation_screened_SSA_ST.png', dpi = 300, transparent = True)       
    ...
    for i, idata in enumerate(list(paras.keys())):
        ...
        plt.savefig(figdir + 'Spatial_vld_AOD.png', dpi = 300, transparent = True)
        print(RMSE(X1, X2), X1.corr(X2), (abs(X1 - X2) <= site.AAOD_err).sum() / len(X1) * 1e2)
...
#%%
    ...
    plt.savefig(figdir + 'Spatial_vld_UVAI.png', dpi = 300, transparent = True)
    ...
    for i, idata in enumerate(list(paras.keys())):
        ...
        plt.savefig(figdir + 'Spatial_vld_SSA.png', dpi = 300, transparent = True)
...
#%%
    ...
    for j, iROI in enumerate(ROIs.keys()): 
        ...
        ax.yaxis.get_major_formatter().set_powerlimits((0,1))
    plt.savefig(figdir + 'Temporal_vld.png', dpi = 300, transparent = True)  
...
for k, itype in enumerate(['Smoke', 'Dust', 'Mixed', 'Other']):
    ...
    for i, idata in enumerate(paras.keys()):
        ...
        plt.scatter(X1, X2, color = colors[i], label = labels[i],
                    marker = markers[i], edgecolor = '', alpha = alphas[i])
